...
Cursor must **never** mark a task DONE automatically.

Cursor must only work on **one** backlog item per session. Do not start another task unless explicitly instructed.

After implementing a task:
1. Set Status: **IN_REVIEW**
2. Summarize the changes
3. Wait for human approval

Only the human reviewer marks Status: **DONE**.
...
|----------|------|--------|
| P1 | Admin list: topic filter — filter questions by topic (URL param + dropdown or links) | DONE |
| P2 | Admin: Create question — button, form (reuse edit form), POST API | DONE |
| P3 | Admin list: show times_asked (from attempts by slug), then add sort by it | DONE |
| P4 | Question rating: allow user to rate (1–5) after an attempt; POST API + UI | DONE |
| P5 | Admin detail: Stats block — times_asked, last asked | DONE |
| P6 | Admin list: difficulty and active filters | DONE |
| P7 | Admin list: theme and dimension filters (URL param + dropdowns) | DONE |
| P8 | Admin list: tag search filter (e.g. questions whose tags contain search term) | DONE |
| P9 | Admin list: sorts by created_at, avg_score, avg_rating | DONE |
| P10 | Admin list: columns avg_score, avg_rating, flags_count | DONE |
| P11 | Admin detail: score distribution (e.g. bar chart 0–4 from attempts) | DONE |
| P12 | Admin detail: feedback — ratings summary | DONE |
| P13 | Admin detail: feedback — comments (Insights / list) | IN_REVIEW |
| P14 | Admin detail: feedback — flags (list open flags with reasons) | DONE |
| P15 | Admin detail: edit topic/theme (currently fixed) | IN_REVIEW |
| P16 | Admin detail: revision history | TODO |
| P17 | User can add comment/insight after attempt (POST API + UI on feedback screen, e.g. next to "Rate this question") | DONE |
